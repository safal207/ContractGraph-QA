...
A watchpoint contains explicit activation conditions and a deterministic step window. Step passage alone cannot activate a watchpoint. Foreign-subject evidence is retained as rejected evidence and cannot activate the pattern. An expired watchpoint does not silently reactivate in the same generation.
