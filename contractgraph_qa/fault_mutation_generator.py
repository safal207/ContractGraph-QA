...
This layer discovers a deliberately small set of syntax-local fault injection
points, binds each generated mutation to reviewed Foundry tests, and emits an
exact Mutation Acquisition v0.1 plan.

Generation is not vulnerability inference and is not proof that a fault model is
complete. Repeated source lines are never mutated by position alone: the engine
expands to the smallest deterministic surrounding source context that is unique in
the exact reviewed source bytes.
"""
...
KNOWN_UNSUPPORTED_FAULT_CLASSES = ("replay_version", "units_decimals")
...
def generator_config_from_dict(data: dict[str, Any]) -> FaultMutationGeneratorConfig:
    ...
    missing = sorted((_MODEL_KEYS - {"scope"}) - set(data))
    ...
    _require(_ACTIVATION_KEYS <= set(raw_activation), "activationWitness missing required fields")
    ...
    _require(_FOUNDRY_KEYS <= set(raw_foundry), "foundry missing required fields")
    timeout = raw_foundry["timeoutSeconds"]
    _require(
        isinstance(timeout, int) and not isinstance(timeout, bool) and 1 <= timeout <= 3600,
        "foundry.timeoutSeconds must be an integer from 1 to 3600",
    )
    foundry = FoundryConfig(
    ...
    raw_bindings = data["testBindings"]
    _require(isinstance(raw_bindings, list) and raw_bindings, "testBindings must be a non-empty array")
    ...
    for index, raw in enumerate(raw_bindings):
        ...
        _require(key not in seen_bindings, f"duplicate test binding for {binding.fault_class}/{binding.function}")
        seen_bindings.add(key)
        bindings.append(binding)

    max_mutations = data["maxMutationsPerFaultClass"]
    _require(
        isinstance(max_mutations, int) and not isinstance(max_mutations, bool) and 1 <= max_mutations <= 50,
        "maxMutationsPerFaultClass must be an integer from 1 to 50",
    )
...
def _binding_for(config: FaultMutationGeneratorConfig, fault_class: str, function_name: str) -> TestBinding | None:
    for item in config.test_bindings:
        if item.fault_class == fault_class and item.function == function_name:
            return item
    for item in config.test_bindings:
        if item.fault_class == fault_class and item.function == "*":
            return item
    return None
...
def _unique_context_mutation(
    ...
    *,
    ...
    """Return the smallest symmetric unique context containing the target line."""

    for radius in range(max_radius + 1):
        start = max(0, target_index - radius)
        end = min(len(lines), target_index + radius + 1)
        match = "".join(lines[start:end])
        if source.count(match) != 1:
            continue
        replacement_lines = list(lines[start:end])
        replacement_lines[target_index - start] = replacement_line
        return match, "".join(replacement_lines), start + 1, end
    return None
...
def generate_fault_mutation_plan(config: FaultMutationGeneratorConfig, project_root: Path) -> dict[str, object]:
    ...
    for index0, line in enumerate(lines):
        ...
        if fault_class not in required:
            continue
        line_number = index0 + 1
        function_name = functions[index0]
        mutation_id = f"auto-{operator_id.lower().replace('_', '-')}-L{line_number}"
        candidate = {
        ...
        if binding is None:
            ...
            continue
        anchored = _unique_context_mutation(
            source,
            lines,
            index0,
            _comment_replacement(line, operator_id),
        )
        if anchored is None:
            ambiguous.append({**candidate, "reason": "no unique source context found within 8 lines"})
            continue
        ...
        generated.append(
            {
                ...
                "faultClass": fault_class,
                "description": (
                    f"{description} in {function_name}() at line {line_number}; "
    ...
    represented = sorted({str(item["faultClass"]) for item in generated})
    ...
    result_core: dict[str, object] = {
        ...
        "claimBoundary": (
            ...
            "unique source contexts, and reviewed test bindings. PASS means every declared required class is supported "
            "by this generator and has at least one executable generated mutation. It does not mean the generated set "
            "is exhaustive, that the fault model is complete, or that any generated mutant is detected; Mutation "
            "Acquisition and CGQ-SPEC-001 establish those later evidence claims."
        ),
    }
