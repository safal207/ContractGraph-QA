"""CLI for deterministic idea/plan/fact graph comparison."""

from __future__ import annotations

import argparse
import json
from pathlib import Path
from typing import Any

from contractgraph_qa.evidence_io import write_text_atomic
from contractgraph_qa.graph_layers import compare_graph_layers, load_graph_layers


def _parser() -> argparse.ArgumentParser:
    parser = argparse.ArgumentParser(
        prog="cgqa-graph-layers",
        description="Compare a declared idea/plan graph with observed fact edges.",
    )
    parser.add_argument("--input", type=Path, required=True, help="Graph-layer JSON")
    parser.add_argument("--output", type=Path, help="Optional diff JSON destination")
    parser.add_argument(
        "--force", action="store_true", help="Replace an existing output (never the input graph)"
    )
    return parser


def _stable(data: Any) -> str:
    return json.dumps(data, indent=2, ensure_ascii=False, sort_keys=True) + "\n"


def main(argv: list[str] | None = None) -> int:
    args = _parser().parse_args(argv)
    try:
        input_path = args.input.resolve()
        result = compare_graph_layers(load_graph_layers(input_path))
        rendered = _stable(result)
        if args.output is not None:
            output_path = args.output.resolve()
            if output_path == input_path or (
                output_path.exists() and output_path.samefile(input_path)
            ):
                raise ValueError("--output must not overwrite the input graph")
            write_text_atomic(output_path, rendered, force=args.force)
        print(rendered, end="")
        return 0
    except (OSError, ValueError, json.JSONDecodeError) as exc:
        _parser().error(str(exc))
        return 2


if __name__ == "__main__":
    raise SystemExit(main())
