...
def validate_manifest(manifest: dict[str, Any]) -> dict[str, Any]:
    ...
    _require(isinstance(execution.get("dry_run_default"), bool), "execution.dry_run_default must be boolean")
    max_concurrency = execution.get("max_concurrency")
    max_depth = execution.get("max_depth")
    max_paths = execution.get("max_paths")
    _require(isinstance(max_concurrency, int) and 1 <= max_concurrency <= MAX_CONCURRENCY_HARD,
             f"max_concurrency must be within 1..{MAX_CONCURRENCY_HARD}")
    _require(isinstance(max_depth, int) and 1 <= max_depth <= MAX_DEPTH_HARD,
             f"max_depth must be within 1..{MAX_DEPTH_HARD}")
    _require(isinstance(max_paths, int) and 1 <= max_paths <= MAX_PATHS_HARD,
    ...
    _require(all(isinstance(item, str) and item for item in env_vars), "credential env-var names must be non-empty strings")
    ...
    _require(all(isinstance(item, str) and item for item in events), "supported_events must contain non-empty strings")
    ...
    _require(all(isinstance(item, str) and item for item in required_values), "required state fields must be non-empty strings")
    ...
    _require(all(isinstance(item, str) and item for item in required_fields), "required evidence fields must be non-empty strings")

    # Deliberately reject common literal-secret containers. Environment variable
    # names are allowed; secret values are not part of this contract.
    for forbidden_key in ("api_key", "token", "secret", "password", "private_key", "credentials_value", "headers"):
        _require(forbidden_key not in manifest, f"literal secret/container field is forbidden in manifest: {forbidden_key}")
...
def validate_snapshot(snapshot: dict[str, Any], manifest: dict[str, Any]) -> None:
    ...
    _require(isinstance(snapshot.get("state_id"), str) and snapshot["state_id"], "snapshot.state_id is required")
...
def validate_observation(observation: dict[str, Any], event: str, manifest: dict[str, Any]) -> None:
    ...
    _require(event in manifest["capabilities"]["supported_events"], f"event not allowed by adapter manifest: {event}")
    ...
    _require(observed_action == event, f"observation request.action mismatch: expected {event}, got {observed_action}")
...
class ContractBoundAdapter:
    ...
    def apply(self, event: str) -> dict[str, Any]:
        _require(event in self.manifest["capabilities"]["supported_events"], f"event not allowed by adapter manifest: {event}")
