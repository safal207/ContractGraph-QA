...
class Phase2TemporalEvidenceTest(unittest.TestCase):
    def setUp(self) -> None:
    ...
    def _witness(self, *, external_ids: list[str] | None = None) -> dict[str, object]:
        ...
        return {
            ...
            "external": {
                ...
                "subjectHash": self.subject_hash,
                "events": [
                    {"eventId": event_id, "objectId": "escrow-1"} for event_id in external_ids
                ],
            },
        }
    ...
    def test_equal_counts_do_not_hide_event_id_gap(self) -> None:
        result = evaluate_independent_witness(self._witness(external_ids=["e1", "e3"]))
    ...
    def _debt(self, status: str) -> dict[str, object]:
        return {
    ...
    def test_completed_is_not_pass_for_required_debt(self) -> None:
        result = evaluate_verification_debt(self._debt("COMPLETED"))
        self.assertEqual(result["status"], "hold")
        self.assertEqual(result["orientationImpact"], "INDETERMINATE")

    def test_completed_pass_resolves_required_debt(self) -> None:
        self.assertEqual(evaluate_verification_debt(self._debt("COMPLETED_PASS"))["status"], "pass")
    ...
    def test_expired_watchpoint_does_not_reactivate(self) -> None:
        model = self._watch_model()
        model["watchpoints"][0]["status"] = "EXPIRED"
        model["evidence"] = [
            {
                "id": "obs-1",
                "step": 5,
        ...
        result = evaluate_causal_watchpoints(model)
        self.assertEqual(result["watchpoints"][0]["status"], "EXPIRED")
    ...
    def test_reused_evidence_is_not_fresh(self) -> None:
        ...
        self.assertIn("EVIDENCE_REUSED", evaluate_replication_drift(model)["freshnessFailures"])

    def test_external_mode_requires_distinct_source(self) -> None:
        model = self._replication()
        model["replication"]["sourceId"] = "source-a"
        self.assertIn(
            "EXTERNAL_SOURCE_NOT_DISTINCT",
            evaluate_replication_drift(model)["freshnessFailures"],
        )
    ...
    def test_history_rewrite_is_rejected(self) -> None:
        ...
        self.assertIn(
            "HISTORY_REWRITE_OR_NON_FORWARD_RESULT",
            evaluate_forward_remediation(model)["reasons"],
        )
    ...
    def test_automatic_rollback_is_rejected(self) -> None:
        ...
        self.assertIn(
            "AUTOMATIC_REMEDIATION_NOT_AUTHORIZED",
            evaluate_forward_remediation(model)["reasons"],
        )
        ...
        orientation = evaluate_orientation_center(
            {
                ...
                "supportingEvidence": [{"id": "historical-pass", "status": "PASS"}],
                "counterevidence": [],
